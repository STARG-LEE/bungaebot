...
const GREETING_TEXT = '안녕하세요! 저는 회계 학습을 도와주는 분개봇이에요. 회계 용어·분개 문제·세법 개념을 단계별로 쉽게 풀어드릴게요. 무엇이 궁금하세요?'
const GREETING_TTS = '안녕하세요! 저는 분개봇이에요. 회계 용어부터 분개, 세법까지 쉽게 풀어드릴게요. 무엇이 궁금하세요?'
...
function normalizeTtsText(text) {
  ...
  return String(text)
    ...
    .replace(/\bT계정\b/g, '티 계정')
    .replace(/\bNRV\b/gi, '엔알브이')
    .replace(/\bFIFO\b/gi, '선입선출')
    .replace(/\s+/g, ' ')
    .trim()
}
